...
class HMapStructure
  ...
  def self.new_from_bin(swapped, bin)
    format = HMapUtils.specialize_format(self::FORMAT, swapped)
...
class HMapHeader < HMapStructure
  ...
  def serialize
    format = HMapUtils.specialize_format(FORMAT, SWAPPED)
...
class HMapData
  ...
  def initialize(buckets)
    super()
    count = buckets.count
    nums = num_buckets(count, HMapUtils.next_power_of_two(count))
  ...
  def num_buckets(count, pow2)
    if count < 8
      ...
      HMapUtils.next_power_of_two(count * 3 + padding)
    end
  end
  ...
  def add_bucket(buckets, num)
    buckets.each_with_object(Array.new(num)) do |bucket, sum|
      ...
      i = HMapUtils.index_of_range(bucket.uuid, num)
      loop do
        sum[i] = serialize if sum[i].nil?
        break if serialize == sum[i]

        i = HMapUtils.index_of_range(i += 1, num)
      end
    end
  end
